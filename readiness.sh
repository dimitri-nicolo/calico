...
retry_count_current=$(curl -s http://localhost:24220/api/plugins.json | jq --raw-output '.plugins[] | select(.config.index_name=="tigera_secure_ee_flows") | .retry_count');
buffer_length_current=$(curl -s http://localhost:24220/api/plugins.json | jq --raw-output '.plugins[] | select(.config.index_name=="tigera_secure_ee_flows") | .buffer_total_queued_size');
