...
var _ = Describe("", func() {
	...
	Context("test label handler", func() {
		It("Labels", func() {
			...
			labels, err := l.GetNamespacesLabels(permissions, context.Background())
			Expect(err).NotTo(HaveOccurred())
			Expect(labels.GetLabels()).To(HaveLen(0))

			By("getting labels for ServiceAccounts")
			labels, err = l.GetServiceAccountsLabels(permissions, context.Background())
			Expect(err).NotTo(HaveOccurred())
			Expect(labels.GetLabels()).To(HaveLen(0))

			By("getting labels for GlobalThreatFeeds")
			labels, err = l.GetGlobalThreatfeedsLabels(permissions, context.Background())
			Expect(err).NotTo(HaveOccurred())
			Expect(labels.GetLabels()).To(HaveLen(1))

			By("getting labels for ManagedClusters")
			labels, err = l.GetManagedClustersLabels(permissions, context.Background())
			Expect(err).NotTo(HaveOccurred())
			Expect(labels.GetLabels()).To(HaveLen(1))
		})
	})
