...
type AuthenticationReviewInformer interface {
	...
	Lister() internalversion.AuthenticationReviewLister
}
...
func NewAuthenticationReviewInformer(client internalclientset.Interface, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
...
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredAuthenticationReviewInformer(client internalclientset.Interface, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.Projectcalico().AuthenticationReviews().List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.Projectcalico().AuthenticationReviews().Watch(context.TODO(), options)
			},
		},
		&projectcalico.AuthenticationReview{},
		resyncPeriod,
		indexers,
	)
}

func (f *authenticationReviewInformer) defaultInformer(client internalclientset.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
...
func (f *authenticationReviewInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&projectcalico.AuthenticationReview{}, f.defaultInformer)
}

func (f *authenticationReviewInformer) Lister() internalversion.AuthenticationReviewLister {
	return internalversion.NewAuthenticationReviewLister(f.Informer().GetIndexer())
}
