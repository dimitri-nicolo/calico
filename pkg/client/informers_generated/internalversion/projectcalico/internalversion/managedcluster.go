...
type ManagedClusterInformer interface {
	...
	Lister() internalversion.ManagedClusterLister
}
...
func NewManagedClusterInformer(client internalclientset.Interface, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
...
// Always prefer using an informer factory to get a shared informer instead of getting an independent
// one. This reduces memory footprint and number of connections to the server.
func NewFilteredManagedClusterInformer(client internalclientset.Interface, resyncPeriod time.Duration, indexers cache.Indexers, tweakListOptions internalinterfaces.TweakListOptionsFunc) cache.SharedIndexInformer {
...
func (f *managedClusterInformer) defaultInformer(client internalclientset.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
...
func (f *managedClusterInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&projectcalico.ManagedCluster{}, f.defaultInformer)
}

func (f *managedClusterInformer) Lister() internalversion.ManagedClusterLister {
	return internalversion.NewManagedClusterLister(f.Informer().GetIndexer())
}
