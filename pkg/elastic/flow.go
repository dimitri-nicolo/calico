...
func getFlowEndpointType(flowLogEndpointType, endpointName string) (flowEndpointType string) {
	...
	case flow.FlowLogEndpointTypeHEP:
		flowEndpointType = resources.TypeCalicoHostEndpoints.Kind
	case flow.FlowLogEndpointTypeWEP:
		flowEndpointType = resources.TypeK8sPods.Kind
	case flow.FlowLogEndpointTypeNetworkSet:
		flowEndpointType = resources.TypeCalicoGlobalNetworkSets.Kind
	case flow.FlowLogEndpointTypeNetwork:
		switch endpointName {
		case flow.FlowLogNetworkPublic:
			flowEndpointType = apiv3.KindFlowPublic
		case flow.FlowLogNetworkPrivate:
...
func getFlowEndpointNamespace(ns string) (flowEndpointNamespace string) {
	flowEndpointNamespace = ns
	if ns == flow.FlowLogGlobalNamespace {
		flowEndpointNamespace = ""
	}
	return
}
	...
	go func() {
		...
		for {
			...
			if err != nil {
				...
				resultChan <- &flow.FlowLogResult{Err: err}
				return
			}
