...
var _ = Describe("Compliance elasticsearch report list tests", func() {
	...
	addReport := func(typeName, name string) *report.ArchivedReportData {
	...
	waitForReports := func() {
		get := func() error {
			r, err := elasticClient.RetrieveArchivedReportSummaries(context.Background(), report.QueryParams{})
			if err != nil {
				return err
			}
	...
	ensureUTC := func(reps []*report.ArchivedReportData) {
		for ii := range reps {
	...
	It("should retrieve no reportTypeName/reportName combinations when no reports are added", func() {
		...
		r, err := elasticClient.RetrieveArchivedReportTypeAndNames(context.Background(), report.QueryParams{})
	...
	It("should retrieve the correct set of reportTypeName/reportName combinations", func() {
		...
		r, err := elasticClient.RetrieveArchivedReportTypeAndNames(cxt, report.QueryParams{})

		By("checking we have the correct set of unique combinations")
		Expect(err).NotTo(HaveOccurred())
		Expect(r).To(HaveLen(6))
		Expect(r).To(ConsistOf(
			report.ReportTypeAndName{"type1", "report1"},
			report.ReportTypeAndName{"type2", "report1"},
			report.ReportTypeAndName{"type1", "report2"},
			report.ReportTypeAndName{"type3", "report3"},
			report.ReportTypeAndName{"type3", "report2"},
			report.ReportTypeAndName{"type4", "report3"},
		))

		By("retrieving the set of unique reportTypeName/reportName combinations with report filter")
		r, err = elasticClient.RetrieveArchivedReportTypeAndNames(cxt, report.QueryParams{
			Reports: []report.ReportTypeAndName{{"type1", ""}, {"", "report2"}, {"type3", "report3"}},
		})
		...
		cancel()
		_, err = elasticClient.RetrieveArchivedReportTypeAndNames(cxt, report.QueryParams{})
	...
	It("should handle more than DefaultPageSize combinations of reportTypeName/reportName", func() {
		...
		var unique []report.ReportTypeAndName
		...
		for ii := 0; ii < DefaultPageSize*2; ii++ {
			...
			_ = addReport(tn, rn)
			_ = addReport(tn, rn)
			unique = append(unique, report.ReportTypeAndName{tn, rn})
		}
		waitForReports()
	...
	It("should retrieve the correct set of reports", func() {
		...
		cxt, cancel := context.WithCancel(context.Background())
		r, err := elasticClient.RetrieveArchivedReportSummaries(cxt, report.QueryParams{
			SortBy: []report.SortBy{{"startTime", false}},
		})
		...
		Expect(r.Reports).To(Equal([]*report.ArchivedReportData{r6, r5, r4, r3, r2, r1}))

		By("retrieving the full set of report summaries (sort by ascending startTime)")
		r, err = elasticClient.RetrieveArchivedReportSummaries(cxt, report.QueryParams{
			SortBy: []report.SortBy{{"startTime", true}},
		})
		...
		Expect(r.Count).To(Equal(6))
		ensureUTC(r.Reports) // Normalize the times to make them comparable.
		Expect(r.Reports).To(Equal([]*report.ArchivedReportData{r1, r2, r3, r4, r5, r6}))

		By("retrieving the full set of report summaries (sort by ascending endTime)")
		r, err = elasticClient.RetrieveArchivedReportSummaries(cxt, report.QueryParams{
			SortBy: []report.SortBy{{"endTime", true}},
		})
		...
		r, err = elasticClient.RetrieveArchivedReportSummaries(cxt, report.QueryParams{
			SortBy: []report.SortBy{{"generationTime", false}}, // generationTime is in opposite order to start/end times
		})
		...
		r, err = elasticClient.RetrieveArchivedReportSummaries(cxt, report.QueryParams{
			SortBy: []report.SortBy{{"reportTypeName", false}, {"startTime", false}},
		})
		...
		Expect(r.Count).To(Equal(6))
		ensureUTC(r.Reports) // Normalize the times to make them comparable.
		Expect(r.Reports).To(Equal([]*report.ArchivedReportData{r6, r5, r4, r2, r3, r1}))

		By("retrieving the full set of report summaries (sort by ascending reportName and descending startTime), maxItems=4")
		maxItems := 4
		r, err = elasticClient.RetrieveArchivedReportSummaries(cxt, report.QueryParams{
			SortBy:   []report.SortBy{{"reportName", true}, {"startTime", false}},
			MaxItems: &maxItems,
		})
		...
		Expect(r.Count).To(Equal(6))
		ensureUTC(r.Reports) // Normalize the times to make them comparable.
		Expect(r.Reports).To(Equal([]*report.ArchivedReportData{r2, r1, r5, r3}))
		...
		Expect(r.Count).To(Equal(6))
		ensureUTC(r.Reports) // Normalize the times to make them comparable.
		Expect(r.Reports).To(Equal([]*report.ArchivedReportData{r6, r4}))

		By("checking we handle cancelled context")
		cancel()
		_, err = elasticClient.RetrieveArchivedReportSummaries(cxt, report.QueryParams{})
		Expect(err).To(HaveOccurred())
	})
	...
	It("should handle the default sort order when start times are the same, sorting by time, type, name", func() {
		...
		ensureUTC(r.Reports) // Normalize the times to make them comparable.
		Expect(r.Reports).To(Equal([]*report.ArchivedReportData{r7, r1, r3, r2, r5, r4, r6}))
	})
})
