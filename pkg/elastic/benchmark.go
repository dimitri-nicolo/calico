...
func (c *client) RetrieveLatestBenchmarks(ctx context.Context, ct benchmark.BenchmarkType, filters []benchmark.Filter, start, end time.Time) <-chan benchmark.BenchmarksResult {
	...
	go func() {
		...
		seen := make(map[string]*benchmark.Benchmarks)
		...
		for {
			...
			if err != nil {
				...
				ch <- benchmark.BenchmarksResult{Err: err}
				return
			}
			if res == nil {
			...
			for _, hit := range res.Hits.Hits {
				benchmarks := new(benchmark.Benchmarks)
				...
				if benchmarks.Error == "" {
					...
					ch <- benchmark.BenchmarksResult{Benchmarks: benchmarks}
				}
			}
		}
...
func getBenchmarksQuery(ct benchmark.BenchmarkType, filters []benchmark.Filter, start, end time.Time) elastic.Query {
...
func getBenchmarksFiltersQuery(filters []benchmark.Filter) elastic.Query {
	queries := []elastic.Query{}
...
func getBenchmarksFilterQuery(filter benchmark.Filter) elastic.Query {
	queries := []elastic.Query{}
	if filter.Version != "" {
