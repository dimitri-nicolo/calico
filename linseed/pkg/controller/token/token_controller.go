...
func (c *controller) needsUpdate(log *logrus.Entry, cs kubernetes.Interface, mcName, tokenName, user, namespace string) (bool, error) {
	...
	if err != nil && !k8serrors.IsNotFound(err) {
		...
		expectedSubject := GenerateSubjectLinseed(c.tenant, mcName, namespace, user)
...
func (c *controller) createToken(tenant, cluster string, user UserInfo) ([]byte, error) {
	...
	subj := GenerateSubjectLinseed(tenant, cluster, user.Namespace, user.Name)
...
func GenerateSubjectLinseed(tenant, cluster, namespace, name string) string {
	return fmt.Sprintf("%s:%s:%s:%s", tenant, cluster, namespace, name)
}
