...
func MustGetCatalogue(cfg linseed.ElasticClientConfig, backendType linseed.BackendType, logLevel string, source string) BackendCatalogue {
	...
	case linseed.BackendTypeMultiIndex:
		flowLogBackend = flowbackend.NewFlowLogBackend(esClient, flowInitializer, maxResultsWindow, true)
		auditBackend = auditbackend.NewBackend(esClient, auditInitializer, maxResultsWindow, true)
		bgpBackend = bgpbackend.NewBackend(esClient, bgpInitializer, maxResultsWindow, true)
		reportsBackend = compliancebackend.NewReportsBackend(esClient, defaultInitializer, maxResultsWindow, true)
		snapshotsBackend = compliancebackend.NewSnapshotBackend(esClient, defaultInitializer, maxResultsWindow, true)
		benchmarksBackend = compliancebackend.NewBenchmarksBackend(esClient, defaultInitializer, maxResultsWindow, true)
		dnsLogBackend = dnsbackend.NewDNSLogBackend(esClient, dnsInitializer, maxResultsWindow, true)
		l7LogBackend = l7backend.NewL7LogBackend(esClient, l7Initializer, maxResultsWindow, true)
		runtimeBackend = runtimebackend.NewBackend(esClient, defaultInitializer, maxResultsWindow, true)
		eventBackend = eventbackend.NewBackend(esClient, defaultInitializer, maxResultsWindow, true)
		wafBackend = wafbackend.NewBackend(esClient, defaultInitializer, maxResultsWindow, true)
		ipSetBackend = threatfeedsbackend.NewIPSetBackend(esClient, defaultInitializer, maxResultsWindow, true)
		domainNameSetBackend = threatfeedsbackend.NewDomainNameSetBackend(esClient, defaultInitializer, maxResultsWindow, true)
	case linseed.BackendTypeSingleIndex:
		flowLogBackend = flowbackend.NewSingleIndexFlowLogBackend(esClient, flowInitializer, maxResultsWindow, true, index.WithBaseIndexName(cfg.ElasticFlowLogsBaseIndexName))
		auditBackend = auditbackend.NewSingleIndexBackend(esClient, auditInitializer, maxResultsWindow, true, index.WithBaseIndexName(cfg.ElasticAuditLogsBaseIndexName))
		bgpBackend = bgpbackend.NewSingleIndexBackend(esClient, bgpInitializer, maxResultsWindow, true, index.WithBaseIndexName(cfg.ElasticBGPLogsBaseIndexName))
		reportsBackend = compliancebackend.NewSingleIndexReportsBackend(esClient, defaultInitializer, maxResultsWindow, true, index.WithBaseIndexName(cfg.ElasticComplianceReportsBaseIndexName))
		snapshotsBackend = compliancebackend.NewSingleIndexSnapshotBackend(esClient, defaultInitializer, maxResultsWindow, true, index.WithBaseIndexName(cfg.ElasticComplianceSnapshotsBaseIndexName))
		benchmarksBackend = compliancebackend.NewSingleIndexBenchmarksBackend(esClient, defaultInitializer, maxResultsWindow, true, index.WithBaseIndexName(cfg.ElasticComplianceBenchmarksBaseIndexName))
		dnsLogBackend = dnsbackend.NewSingleIndexDNSLogBackend(esClient, dnsInitializer, maxResultsWindow, true, index.WithBaseIndexName(cfg.ElasticDNSLogsBaseIndexName))
		l7LogBackend = l7backend.NewSingleIndexL7LogBackend(esClient, l7Initializer, maxResultsWindow, true, index.WithBaseIndexName(cfg.ElasticL7LogsBaseIndexName))
		runtimeBackend = runtimebackend.NewSingleIndexBackend(esClient, defaultInitializer, maxResultsWindow, true, index.WithBaseIndexName(cfg.ElasticRuntimeReportsBaseIndexName))
		eventBackend = eventbackend.NewSingleIndexBackend(esClient, defaultInitializer, maxResultsWindow, true, index.WithBaseIndexName(cfg.ElasticAlertsBaseIndexName))
		wafBackend = wafbackend.NewSingleIndexBackend(esClient, defaultInitializer, maxResultsWindow, true, index.WithBaseIndexName(cfg.ElasticWAFLogsBaseIndexName))
		ipSetBackend = threatfeedsbackend.NewSingleIndexIPSetBackend(esClient, defaultInitializer, maxResultsWindow, true, index.WithBaseIndexName(cfg.ElasticThreatFeedsIPSetBaseIndexName))
		domainNameSetBackend = threatfeedsbackend.NewSingleIndexDomainNameSetBackend(esClient, defaultInitializer, maxResultsWindow, true, index.WithBaseIndexName(cfg.ElasticThreatFeedsDomainSetBaseIndexName))
	default:
