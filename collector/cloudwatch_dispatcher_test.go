...
var _ = Describe("CloudWatch Dispatcher verification", func() {
	...
	Context("FlowLog Serialization", func() {
		...
		It("generates the correct FlowLog string", func() {
			...
			flowLog = testSerialize(flowMeta, flowLabels, flowStats, startTime, endTime, false, false)
			...
			flowLog = testSerialize(flowMeta, flowLabels, flowStats, startTime, endTime, true, false)
...
func testSerialize(fm FlowMeta, fl FlowLabels, fs FlowStats, st, et time.Time, labels bool, policies bool) string {
	f := FlowData{fm, FlowSpec{fl, fs}}.ToFlowLog(st, et, labels)
