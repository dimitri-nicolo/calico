...
description: Optionally customize manifests prior to installing Calico Enterprise (recommended).
...
- Start with one of the [Calico for policy and networking]({{site.baseurl}}/reference/other-install-methods/kubernetes/installation/calico) manifests.
...
Refer to the
[{{site.prodname}} ConfigMap manifest]({{site.baseurl}}/manifests/alp/istio-inject-configmap-1.4.2.yaml){:target="_blank"} for an
