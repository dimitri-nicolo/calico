...
| `FlowLogsFileIncludeServices` | `FELIX_FLOWLOGSFILEINCLUDESERVICES` | `false` | Set to `true` to include destination service information in flow logs. The service information is derived from pre-DNAT destination IP and is therefore only available on the node where DNAT occurs. This parameter only takes effect when `FlowLogsFileEnabled` is set to `true`. |
