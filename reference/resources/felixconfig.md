...
| awsSecondaryIPSupport              | Controls whether Felix will create secondary AWS ENIs for AWS-backed IP pools.  This feature is documented in the [egress gateways on AWS guide](../../networking/egress-gateway-aws).  Should only be enabled on AWS. | `Enabled`, `Disabled` | string | `Disabled` |
