...
type IPSet interface {
	...
	// wire to elastic.
	PutIPSet(name string, set []string) error
	GetIPSet(name string) ([]string, error)
}

type SuspiciousIP interface {
	QueryIPSet(name string) ([]FlowLog, error)
}

type Events interface {
	PutFlowLog(FlowLog) error
}
...
func (e *Elastic) PutIPSet(name string, set []string) error {
	err := e.ensureIndexExists(IPSetIndex, ipSetMapping)
	if err != nil {
		return err
	}
	...
	// Put document
	body := ipsetDoc{set}
	_, err = e.c.Index().Index(IPSetIndex).Type(StandardType).Id(name).BodyJson(body).Do(context.Background())
	log.WithField("name", name).Info("IP set stored")

	return err
}

func (e *Elastic) ensureIndexExists(idx, mapping string) error {
	// Ensure Index exists
	exists, err := e.c.IndexExists(idx).Do(context.Background())
	if err != nil {
		return err
	}
	if !exists {
		r, err := e.c.CreateIndex(idx).Body(mapping).Do(context.Background())
		if err != nil {
			return err
		}
		if !r.Acknowledged {
...
func (e *Elastic) QueryIPSet(name string) ([]FlowLog, error) {
	q := elastic.NewTermsQuery("source_ip").TermsLookup(
		elastic.NewTermsLookup().
			...
			Id(name).
			Path("ips"))
	r, err := e.c.Search().Index(FlowLogIndex).Query(q).Size(1000).Do(context.Background())
	if err != nil {
		return nil, err
	}
...
func (e *Elastic) PutFlowLog(f FlowLog) error {
	err := e.ensureIndexExists(EventIndex, eventMapping)
	if err != nil {
		return err
	}

	_, err = e.c.Index().Index(EventIndex).Type(StandardType).Id(f.id()).BodyJson(f).Do(context.Background())
	return err
}
