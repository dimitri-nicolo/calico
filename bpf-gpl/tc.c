...
static CALI_BPF_INLINE int calico_tc(struct __sk_buff *skb)
{
	...
	if (ctx.state->ip_proto == IPPROTO_UDP) {
		...
		if ((ctx.state->ct_result.flags & CALI_CT_FLAG_TRUST_DNS) &&
			...
			// AAAA response it won't be able to calculate a latency.  Because
			// we want to allow both inbound and outbound packets in this
...
skip_pre_dnat_default:
	if (rt_addr_is_local_host(ctx.state->post_nat_ip_dst)) {
		...
		if (CALI_F_FROM_HEP && is_failsafe_in(ctx.state->post_nat_dport)) {
	...
	if (rt_addr_is_local_host(ctx.state->ip_src)) {
		...
		if (CALI_F_TO_HEP && is_failsafe_out(ctx.state->post_nat_dport)) {
