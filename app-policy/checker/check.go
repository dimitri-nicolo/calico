...
func Evaluate(scope PolicyScope, dir rules.RuleDir, store *policystore.PolicyStore, ep *proto.WorkloadEndpoint, flow Flow, traceBuf []*calc.RuleID) ([]*calc.RuleID, error) {
	if store != nil && store.Verdicts != nil {
		if key, ok := verdictKey(store, scope, dir, ep, flow); ok {
			...
			trace, err := evaluate(scope, dir, store, ep, flow, traceBuf)
			if err == nil {
				// The caller may reuse traceBuf; the cache keeps its own copy.
				store.Verdicts.Store(store.Generation, key, append([]*calc.RuleID(nil), trace...))
			}
			return trace, err
		}
	}
	return evaluate(scope, dir, store, ep, flow, traceBuf)
}
