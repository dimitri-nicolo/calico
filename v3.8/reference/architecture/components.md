...
various components required for networking containers with {{site.tseeprodname}}.  The key
components are:
...
The Felix daemon is the heart of {{site.tseeprodname}} networking.  Felix's primary job is to
...
{{site.tseeprodname}} uses a fork of the main BIRD repo, to include an additional feature
required for IPIP support when running {{site.tseeprodname}} in a cloud environment.  Refer
...
{{site.tseeprodname}} uses a fork of the main confd repo which includes an additional change
