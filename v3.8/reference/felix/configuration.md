...
| `Ipv6Support`                     | `FELIX_IPV6SUPPORT`                     | Enable {{site.tseeprodname}} networking and security for IPv6 traffic as well as for IPv4. | boolean |
...
| `IptablesMarkMask`                   | `FELIX_IPTABLESMARKMASK`                   | Mask that Felix selects its IPTables Mark bits from. Should be a 32 bit hexadecimal number with at least 8 bits set, none of which clash with any other mark bits in use on the system.  When using {{site.tseeprodname}} with Kubernetes' `kube-proxy` in IPVS mode, [we recommend allowing at least 16 bits](#ipvs-bits). [Default: `0xffff0000`] | netmask |
...
> **Note**: <a id="ipvs-bits"></a> When using {{site.tseeprodname}} with Kubernetes' `kube-proxy` in IPVS mode, {{site.tseeprodname}} uses additional iptables mark bits to store an ID for each local {{site.tseeprodname}} endpoint.
> For example, the default `IptablesMarkMask` value, `0xffff0000` gives {{site.tseeprodname}} 16 bits, up to 6 of which are used for internal purposes, leaving 10 bits for endpoint IDs.
> 10 bits is enough for 1024 different values and {{site.tseeprodname}} uses 2 of those for internal purposes, leaving enough for 1022 endpoints on the host.
{: .alert .alert-info}
...
1. Modify logFilePath to your intended path, e.g. "/tmp/felix.log"
   ...
   > For a node-specific change: set name to the node name, e.g. "{{site.tseeprodname}}-Node-1"
   {: .alert .alert-success}
