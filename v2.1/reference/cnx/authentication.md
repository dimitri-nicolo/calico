...
This document describes the authentication methods supported by {{site.tseeprodname}}
...
{{site.tseeprodname}} doesn't have its own authentication and authorization system, it delegates
...
The {{site.tseeprodname}} Manager web interface allows users to select the authentication method
...
To use Google login, the web server component of {{site.tseeprodname}} Manager needs
...
1. [Setup Google OAuth 2.0](https://developers.google.com/identity/protocols/OpenIDConnect).
   - Ensure the redirect URIs are set to `https://<{{site.tseeprodname}} Manager name>:<port>/login/oidc/callback`.
...
3. Configure {{site.tseeprodname}} Manager to use it
   - Set the client ID in the `tigera-cnx-manager-config` ConfigMap (referenced
     in the installation instructions).

4. If {{site.tseeprodname}} Manager has already been deployed, restart the web server (achieved by deleting the pod).
...
1. Select the **Login via username and password** option in the {{site.tseeprodname}} 
...
The **Login via static token** option tells {{site.tseeprodname}} Manager to pass the token through
