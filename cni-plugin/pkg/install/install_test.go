...
-----END CERTIFICATE-----`)
	...
	It("should write the value of MULTI_INTERFACE_MODE to disk", func(done Done) {
		err := runCniContainer(tempDir, "-e", "MULTI_INTERFACE_MODE=test")
		Expect(err).NotTo(HaveOccurred())
		content, err := ioutil.ReadFile(tempDir + "/net.d/calico_multi_interface_mode")
	...
	It("should remove the calico_multi_interface_mode file if MULTI_INTERFACE_MODE isn't set", func(done Done) {
		Expect(ioutil.WriteFile(tempDir+"/net.d/calico_multi_interface_mode", []byte("test"), 0755)).ShouldNot(HaveOccurred())
		Expect(runCniContainer(tempDir)).NotTo(HaveOccurred())
