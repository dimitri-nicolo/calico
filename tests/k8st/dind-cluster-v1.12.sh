...
function dind::up {
  ...
  case "${CNI_PLUGIN}" in
    ...
    calico)
      ...
      docker cp ./cnx-node.tar $(dind::master-name):/cnx-node.tar
      docker exec $(dind::master-name) docker load -i /cnx-node.tar
      for ((n=1; n <= NUM_NODES; n++)); do
          docker cp ./cnx-node.tar $(dind::node-name ${n}):/cnx-node.tar
          docker exec $(dind::node-name ${n}) docker load -i /cnx-node.tar
      done
      ...
      dind::retry "${kubectl}" --context "$ctx" -n kube-system create secret generic cnx-pull-secret \
	      --from-file=.dockerconfigjson=${GCR_IO_PULL_SECRET} \
	      --type=kubernetes.io/dockerconfigjson

      # Install using manifests which are Calico open source
      # manifests - but using the locally built node image and pretending that
      # it is "calico/node".
