...
func (sr *stagednetworkpoliciesReconciler) Reconcile(namespacedName types.NamespacedName) error {
	log.WithField("key", namespacedName.Name).Debug("Reconciling StagedNetworkPolicy")

	name := fmt.Sprintf("%s.%s", prtypes.PolicyRecommendationTier, namespacedName.Name)
	snp, err := sr.calico.StagedNetworkPolicies(namespacedName.Namespace).Get(context.Background(), name, metav1.GetOptions{})
	...
	if cachedSNP == nil || reflect.ValueOf(cachedSNP).IsNil() {
		log.WithField("key", name).Debug("Ignoring untracked StagedNetworkPolicy")
		return nil
	}

	if err == nil && snp != nil {
		// Sync the recommendation's stagedAction metadata with the store value
		if snp.Spec.StagedAction != cachedSNP.Spec.StagedAction {
			if snp.Spec.StagedAction == v3.StagedActionIgnore || snp.Spec.StagedAction == v3.StagedActionSet {
				...
				cachedSNP.Annotations[calicoresources.StagedActionKey] = string(snp.Spec.StagedAction)
			}
		}
	}
