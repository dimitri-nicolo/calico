...
var _ = Describe("GetOrderedEndpointSubsets UT", func() {

	It("should handle nil input", func() {
		Expect(GetOrderedEndpointSubsets(nil)).To(Equal([]v1.EndpointSubset{}))
	})

	It("should handle a complex sort and deduplication", func() {
		input := []v1.EndpointSubset{
			{
				Addresses: []v1.EndpointAddress{
					{
						IP:       "1.2.3.9",
			...
			// This should be merged between another set of entries.
			{
				Addresses: []v1.EndpointAddress{
					{
						IP:       "1.2.3.5",
		...
		// Order should be name, protocol, ip, port
		// The output should be the fully expanded set of results (i.e. a single address and port in each entry).
		output := []v1.EndpointSubset{
			// a-port is first
			{
				Addresses: []v1.EndpointAddress{
					{
		...
		Expect(GetOrderedEndpointSubsets(input)).To(Equal(output))
	})
})
