...
$(KIBANA_IMAGE):
	...
	docker build --build-arg GTM_INTEGRATION=$(GTM_INTEGRATION) -t $(KIBANA_IMAGE):latest-$(ARCH) --file ./docker/Dockerfile docker/.
ifeq ($(ARCH),amd64)
	docker tag $(KIBANA_IMAGE):latest-$(ARCH) $(KIBANA_IMAGE):latest
endif

compressed-image: image
	$(MAKE) docker-compress IMAGE_NAME=$(KIBANA_IMAGE)
