...
`NetworkPolicy` resources can be used to define network connectivity rules between groups of {{site.tseeprodname}} endpoints and host endpoints, and
...
NetworkPolicies are organised into [tiers]({{site.baseurl}}/{{page.version}}/reference/calicoctl/resources/tier), which provide an additional layer of ordering—in particular, note that the `Pass` action skips to the
...
> **Note**: {{site.tseeprodname}} `NetworkPolicy` resources with `knp.` prefixed names are determined from Kubernetes `NetworkPolicy` 
