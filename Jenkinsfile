    ...
    stages {
        stage('Checkout') {
            steps {
                ...
                git(url: 'git@github.com:tigera/calico-k8sapiserver.git', credentialsId: 'marvin-tigera-ssh-key', branch: "${env.BRANCH_NAME}")
            }
        }
                ...
                script{
                    withCredentials([file(credentialsId: 'wavetank_service_account', variable: 'DOCKER_AUTH')]) {
                        ...
                        if (env.BRANCH_NAME == 'master') {
                            ...
                            sh "docker tag tigera/cnx-apiserver:latest ${env.IMAGE_NAME}:${env.IMAGE_TAG}"
