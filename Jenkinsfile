    ...
    stages {
        stage('Checkout') {
            steps {
                ...
                git(url: 'git@github.com:tigera/typha-private.git', credentialsId: 'marvin-tigera-ssh-key', branch: "${env.BRANCH_NAME}")
            }
        }
                ...
                script{
                    ...
                    withCredentials([file(credentialsId: 'wavetank_service_account', variable: 'DOCKER_AUTH')]) {
                        if (env.BRANCH_NAME == 'master') {
                            ...
                            sh "docker tag tigera/typha:latest ${env.IMAGE_NAME}:${env.IMAGE_TAG}"
