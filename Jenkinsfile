...
pipeline {
    ...
    environment {
        IMAGE_NAME = "gcr.io/unique-caldron-775/cnx/tigera/confd"
    ...
    stages {
        ...
        stage('Clean artifacts, build, and test') {
            steps {
                echo 'running ci..'
            ...
            steps {
                script{
                    withCredentials([file(credentialsId: 'wavetank_service_account', variable: 'DOCKER_AUTH')]) {
                        if (env.BRANCH_NAME ==~ /(master|release-.*)/  && (currentBuild.result == null || currentBuild.result == 'SUCCESS')) {
                            sh "cp $DOCKER_AUTH key.json"
                            sh "gcloud auth activate-service-account ${env.WAVETANK_SERVICE_ACCT} --key-file key.json"
                            sh "gcloud auth configure-docker"
                            sh "docker tag tigera/confd:latest ${IMAGE_NAME}:${BRANCH_NAME}"
