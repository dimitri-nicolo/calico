pipeline{
    ...
    stages {
        ...
        stage('Build tigera/cnx-node') {
            steps {
                sh """
                    ...
                    git clone -b $CHECKOUT_BRANCH git@github.com:$CLONE_ORG/node-private.git && \
                    cd node-private && \
                ...
                script{
                    ...
                    withCredentials([file(credentialsId: 'wavetank_service_account', variable: 'DOCKER_AUTH')]) {
                        if (env.BRANCH_NAME ==~ /(master|release-.*)/) {
                            ...
                            // Clean up images.
                            // Hacky since empty displayed tags are not empty according to gcloud filter criteria
                            sh """
        ...
        stage('Get enterprise calicoctl') {
            steps {
                sh "gcloud compute ssh ubuntu@${BUILD_INSTANCE_NAME} -- 'cd node-private && make dist/calicoctl'"
            }
        }

        stage('Run tigera/cnx-node FVs') {
            steps {
               sh "gcloud compute ssh ubuntu@${BUILD_INSTANCE_NAME} -- 'cd node-private && RELEASE_STREAM=master make st'"
            }
            post {
                always {
                    sh "gcloud compute scp ubuntu@${BUILD_INSTANCE_NAME}:node-private/nosetests.xml nosetests.xml"
                    junit("nosetests.xml")
                }
            }
        }
