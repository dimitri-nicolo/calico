...
- You want to use a single etcd cluster for {{site.tseeprodname}} and Kubernetes (rather than
  having an etcd cluster for {{site.tseeprodname}} and a separate etcd cluster for Kubernetes.
- You want to restrict the read/write access of the various {{site.tseeprodname}} components
...
(like {{site.tseeprodname}} and Kubernetes) will need to authenticate with the etcd cluster.
...
- If using only the v2 API with etcd, as {{site.tseeprodname}} does, then the minimum etcd
...
4. Configure components. For example:
   - [Setting up Kubernetes with {{site.tseeprodname}} utilizing etcd RBAC](kubernetes).
