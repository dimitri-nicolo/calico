...
in [{{site.tseeprodname}} over IP fabrics]({{site.url}}/{{page.version}}/networking/design/l3-interconnect-fabric).
...
Yes.  If you are running in a public cloud that doesn't allow either L3 peering or L2 connectivity between {{site.tseeprodname}} hosts then you can enable IP-in-IP in your {{site.tseeprodname}} IP pool:
...
### Why isn't {{site.tseeprodname}} working on CoreOS Container Linux / hyperkube?

{{site.tseeprodname}} hosted install places the necessary CNI binaries and config on each
...
Without the above volume mounts, the kubelet will not call the {{site.tseeprodname}} CNI binaries, and so
{{site.tseeprodname}} [workload endpoints]({{site.url}}/{{page.version}}/reference/calicoctl/resources/workloadendpoint) will
not be created, and {{site.tseeprodname}} policy will not be enforced.

### How do I view {{site.tseeprodname}} CNI logs?

The {{site.tseeprodname}} CNI plugin emits logs to stderr, which are then logged out by the kubelet.  Where these logs end up
...
When using {{site.tseeprodname}} IPAM, IP addresses are assigned from [IP Pools]({{site.url}}/{{page.version}}/reference/calicoctl/resources/ippool).
...
However, if you do need to assign a particular address to a Pod, {{site.tseeprodname}} provides two ways of doing this:

- You can request an IP that is available in {{site.tseeprodname}} IPAM using the `cni.projectcalico.org/ipAddrs` annotation.
