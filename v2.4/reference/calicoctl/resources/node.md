...
A node resource (`Node`) represents a node running {{site.tseeprodname}}.  When adding a host
to a {{site.tseeprodname}} cluster, a node resource needs to be created which contains the
...
|--------|-----------------------------|-------------------|--------|------------|
| bgp    | BGP configuration for this node.  Omit if using {{site.tseeprodname}} for policy only. | | [BGP](#bgp) |
...
| ipv4Address | The IPv4 address and subnet exported as the next-hop for the {{site.tseeprodname}} endpoints on the host | The IPv4 address must be specified if BGP is enabled. | string |
| ipv6Address | The IPv6 address and subnet exported as the next-hop for the {{site.tseeprodname}} endpoints on the host | Optional | string |
