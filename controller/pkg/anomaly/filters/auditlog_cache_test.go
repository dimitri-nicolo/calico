...
func TestAuditLogCache(t *testing.T) {
	...
	filtered, err := cache.isKeyFiltered(context.TODO(), k1)
	...
	filtered, err = cache.isKeyFiltered(context.TODO(), k2)
	g.Expect(err).ShouldNot(HaveOccurred())
	...
	filtered, err = cache.isKeyFiltered(context.TODO(), k3)
	g.Expect(err).ShouldNot(HaveOccurred())
	g.Expect(filtered).Should(BeTrue())
	...
	_, err = cache.isKeyFiltered(context.TODO(), k4)
	...
	_, err = cache.isKeyFiltered(context.TODO(), k5)
	g.Expect(err).Should(HaveOccurred())
	g.Expect(cache.cache).Should(HaveLen(3))
	...
	// All false
	filtered, err = cache.areKeysFiltered(context.TODO(), k1, k1)
	g.Expect(err).ShouldNot(HaveOccurred())
	g.Expect(filtered).Should(BeFalse())

	// One true
	filtered, err = cache.areKeysFiltered(context.TODO(), k2)
	g.Expect(err).ShouldNot(HaveOccurred())
	g.Expect(filtered).Should(BeTrue())

	// Second true
	filtered, err = cache.areKeysFiltered(context.TODO(), k1, k2)
	g.Expect(err).ShouldNot(HaveOccurred())
	g.Expect(filtered).Should(BeTrue())

	// First true
	filtered, err = cache.areKeysFiltered(context.TODO(), k2, k1)
	g.Expect(err).ShouldNot(HaveOccurred())
	g.Expect(filtered).Should(BeTrue())

	// Second errors
	al.CreatedErr = errors.New("test")
	_, err = cache.areKeysFiltered(context.TODO(), k1, k4)
	g.Expect(err).Should(HaveOccurred())

	// First errors
	_, err = cache.areKeysFiltered(context.TODO(), k4, k1)
	g.Expect(err).Should(HaveOccurred())

	// First true, second errors but is never checked
	filtered, err = cache.areKeysFiltered(context.TODO(), k2, k4)
	g.Expect(err).ShouldNot(HaveOccurred())
	g.Expect(filtered).Should(BeTrue())
}
