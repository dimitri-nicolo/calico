...
type ResourceType struct {
	...
	// to the named client in the supplied map of clients. If a client is not in the supplied map,
	// then the corresponding watcher will not be created (i.e. there will be no updates from that
	// resource type).
	ClientID string
...
func NewMultiClient(
	...
	rs := &watcherSyncer{
		watcherCaches: make([]*resyncCache, 0, len(resourceTypes)),
		results:       make(chan interface{}, 2000),
		callbacks:     callbacks,
	}
	for _, r := range resourceTypes {
		if client, ok := clients[r.ClientID]; ok {
			rs.watcherCaches = append(rs.watcherCaches, newWatcherCache(client, r, rs.results, ticker))
		} else {
...
func (ws *watcherSyncer) Start() {
	...
	// The cancel function is stored off and when called it signals to the caches that they need to
	// wrap up their work.
	// watcher caches wait group (wswc) is used to signal the completion of all of the watcher cache
	// goroutines.
...
func (ws *watcherSyncer) Stop() {
	...
	// Closing the results chan signals to the watchersyncer to shut itself down now that nothing else
	// will write to the results chan
	close(ws.results)
	ws.wgws.Wait()
