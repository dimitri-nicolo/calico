...
1. The bundled Elasticsearch operator is configured to use a `StorageClass` called `elasticsearch-storage` and local storage.
   ...
   ```bash
   {{cli}} apply -f \
   {{site.baseurl}}/{{page.version}}/getting-started/kubernetes/installation/hosted/cnx/1.7/elastic-storage-local.yaml
   ```

   > **Note**: You can also
   > [view the manifest in a new tab]({{site.baseurl}}/{{page.version}}/getting-started/kubernetes/installation/hosted/cnx/1.7/elastic-storage-local.yaml){:target="_blank"}.
   {: .alert .alert-info}
