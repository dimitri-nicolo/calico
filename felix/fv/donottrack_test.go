	...
	Context("after adding host endpoints", func() {
		...
		It("should implement untracked policy correctly", func() {
			...
			if BPFMode() {
				...
				setNameV4 := utils.IPSetNameForSelector(4, host1Selector)
				setNameV6 := utils.IPSetNameForSelector(6, host1Selector)
				domainSetNameV4 := strings.Replace(setNameV4, "cali40s:", "cali40d:", 1)
				domainSetNameV6 := strings.Replace(setNameV6, "cali60s:", "cali60d:", 1)
