...
var (
	...
	stateOffFlags = FieldOffset{Offset: stateEventHdrSize + 416, Field: "state->flags"}
...
func (p *Builder) writeIPSetMatch(negate bool, leg matchLeg, ipSets []string) {
	...
	for _, ipSetID := range ipSets {
		...
		if negate {
			p.b.AddCommentF("If %s matches ipset %s, skip to next rule", leg, ipSetID)
		} else {
			p.b.AddCommentF("If %s doesn't match ipset %s, skip to next rule", leg, ipSetID)
		}
...
func (p *Builder) writeIPSetOrMatch(leg matchLeg, ipSets []string) {
	...
	for _, ipSetID := range ipSets {
		...
		if len(ipSets) == 1 {
			...
			p.b.AddCommentF("If %s doesn't match ipset %s, jump to next ipset", leg, ipSetID)
		}
