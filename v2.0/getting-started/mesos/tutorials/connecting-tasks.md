...
When {{site.tseeprodname}} is used to network Mesos tasks,
...
When using {{site.tseeprodname}}, it needs to be reconfigured to return the tasks' Container IP instead.
...
For example, a {{site.tseeprodname}}-networked task named `nginx` scaled to 3 instances will
respond to DNS lookups with the {{site.tseeprodname}} IPs:
...
The following address will resolve to a task's {{site.tseeprodname}} IP:
...
For accessing {{site.tseeprodname}} tasks from _outside_ the cluster, Marathon-LB can be used to
forward requests on to {{site.tseeprodname}} tasks. The following diagram demonstrates how this works:
...
Additionally, for {{site.tseeprodname}}, you'll need to explicitly tell it which port to load balance to.
...
public slave) will be fulfilled by the {{site.tseeprodname}} task backing it. We can simulate this resolution with a simple curl:
...
requests to this VIP will be round robin'd to the {{site.tseeprodname}} IPs assigned
