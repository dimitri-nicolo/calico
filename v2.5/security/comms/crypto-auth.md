...
![{{site.tseeprodname}} Manager traffic diagram]({{site.url}}/images/cnx-tls-mgr-comms.svg){: width="60%" }
...
proxies etc., between user web browsers and the {{site.tseeprodname}} Manager.  If you do so, configure your proxy
such that {{site.tseeprodname}} Manager receives a HTTPS (TLS) connection, not unencrypted HTTP.
...
If you require TLS termination at any of these proxy elements, you will need to
  ...
  * re-originate a TLS connection from your proxy to {{site.tseeprodname}} Manager, as it expects TLS

If you do not require TLS termination, configure your proxy to "pass thru" the TLS to {{site.tseeprodname}} Manager.
...
Make sure that {{site.tseeprodname}} knows where to read the certificates from by setting the
