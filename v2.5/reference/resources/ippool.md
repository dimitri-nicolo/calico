...
`ippool`, `ippools`, `ipp`, `ipps`, `pool`, `pools`.
...
| nodeSelector | Selects the nodes that {{site.tseeprodname}} IPAM should assign addresses from this pool to. | | [selector](#node-selector) | all() |

> **Important**: Do not use a custom `blockSize` until **all** {{site.tseeprodname}} components have been updated to a version that
...
{{site.tseeprodname}} will only route using VXLAN if the IP address of the destination node is in a different
subnet. The subnet of each node is configured on the node resource (which may be automatically
determined when running the `{{site.nodecontainer}}` service).
...
> `nodeSelector`. This means that {{site.tseeprodname}} IPAM will not allocate any
