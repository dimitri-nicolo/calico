...
func validateGlobalAlertTemplateName(structLevel validator.StructLevel) {
	...
	if specs.Type == v3.GlobalAlertTypeAnomalyDetection && ADDetectorsSet()[specs.Detector] {
		templateObjName := structLevel.Current().Interface().(v3.GlobalAlertTemplate).Name
		expectedName := GlobalAlertDetectorTemplateNamePrefix + strings.Replace(specs.Detector, "_", "-", -1)
		if templateObjName != expectedName && ADDetectorsGlobalAlertTemplateNameSet()[expectedName] {
			...
			structLevel.ReportError(
				...
				"Detector",
				"",
				reason(fmt.Sprintf("a GlobalAlertTemplate for Detector %s already exists", specs.Detector)),
				"",
			)
		}
	}
